...
for j in range(2, len(a) + 1):
    ...
    while i > 0 and read_array(a, i) > key:
        #a[i+i] = a[i]
        write_and_print_array(a, i+1, read_array(a, i), width, height, canvas, i, sleep_seconds=0.5)

        i = i - 1

    #a[i+1] = key
    write_and_print_array(a, i+1, key, width, height, canvas, j, sleep_seconds=0.5)
